...
81 olarak bulduğumuz sonuç tam olarak 81 değil. Bunun nedeni 81 sayısının (aslında tüm sayıların) ikili sistem kullanan bilgisayarlarda tam olarak temsil edilememesidir.  81'in temsilindeki hata: 
```{r}
...
```{r}
save(score, gender, B, b, x, file = "Data/verilerim1.RData")
```

Yeniden kullanmak istediğimizde:  
```{r}
load("Data/verilerim1.RData")
```
...
Temel R'da bunun için `curve()` kullanılabilir. Yardım dosyasına ulaşmak için bkz. `?curve`.  
