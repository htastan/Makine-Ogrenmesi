...
# Tidymodels Kütüphaneleri


[Tidymodels](https://www.tidymodels.org/) makine öğrenmesi modellerinin kolayca eğitilmesi ve değerlendirilmesi için oluşturulmuş paketler topluluğudur. Tidyverse yaklaşımını takip eden bu paketler şunlardır:  

- `{rsample}`: yeniden örnekleme

- `{recipes}`: kestirim değişkenlerinin belirlenmesi

- `{parsnip}`: modelin eğitilmesi

- `{tune}`, `{dials}`: modelin hiperparametrelerinin belirlenmesi 

- `{yardstick}`: modelin performansının değerlendirilmesi 

![](img/tidymodels1.PNG) 

Giriş için bkz. [https://www.tidymodels.org/start/](https://www.tidymodels.org/start/)
...
Örnek olarak `Auto` veri setini kullanarak bir regresyon modeli tahmin edelim. 

Önce verileri eğitim ve test olmak üzere iki kısma ayıralım. Bunun için `{rsample}` paketindeki `initial_split()` fonksiyonunu kullanabiliriz. 

Çıktı değişkenimiz `mpg` kestirim değişkenimiz ise `horsepower`. 
Verilerin rassal seçilmiş yüzde 75'ini eğitim, %25'ini de test verisi olarak kullanacağız.  
```{r}
library(tidymodels)
library(ISLR)
set.seed(1)
...
Yukarıda `strata` opsiyonu ile bağımlı değişkenin hem eğitim hem de test verilerinde aşağı yukarı benzer bir dağılıma sahip olması sağlanabilir.   
...
head(auto_test)
```
`auto_test` verileri en sonda sadece model değerlendirmesinde kullanılacak.  

`mpg`'nin `horsepower` üzerine regresyonunda `{parsnip}` paketi kullanılabilir. Adımlar: 

1. Model türünü belirle:  `parsnip::linear_reg()`. 

2. Model tahmininde kullanılacak fonksiyonu belirle (engine,  `lm()` fonksiyonu): `parsnip::set_engine('lm')`. 

3. Gözetimli öğrenme türünü belirle (sınıflandırma ya da regresyon) `parsnip::set_mode('regression')`. 

Tahmin nesnesini oluştur: `lm_model`
```{r}
lm_model <- linear_reg() %>%
  set_engine('lm') %>%
  set_mode('regression')
...
`parsnip::fit()` fonksiyonu ile modeli eğit: 
```{r}
auto_fit <- lm_model %>%
...
Test verilerini kullanarak kestirim değerlerini oluştur: 
```{r}
auto_preds <- auto_fit %>%
  predict(new_data = auto_test)
```

`auto_preds` nesnesinin elemanları: 
```{r}
head(auto_preds)
```

`.pred` sütünunda yer alan değerler test verileriyle oluşturulmuştur.  


Test setinde yer alan `mpg` ve `horsepower` verileriyle birleştir: 
```{r}
...
`{yardstick}` ile değerlendirme metriklerini hesapla. Burada `rmse()` ölçütünü kullanacağız: 
```{r}
auto_test_results %>% 
  rmse(truth = mpg, estimate = .pred)
```

Bunu hesaplamak için gerçek değerlere ve kestirim değerlerine ihtiyaç duyuyoruz. 

Gerçek ile kestirim arasındaki R-squared de ölçüt olabilir:  
```{r}
auto_test_results %>%
...
Başarılı bir kestirim modeli için noktaların 45 derece doğrusu üzerinde dizilmesini bekleriz. Bu örnekte böyle olmadığını görüyoruz. 

Modele karesel terimler ekleyelim: 
```{r}
auto_fit2 <- lm_model %>%
...
`{yardstick}` paketiyle metrikleri hesapla:  
```{r}
auto_test_results2 %>% 
  rmse(truth = mpg, estimate = .pred)
```

Bu model birinci modele göre daha başarılıdır.  
...
R-kare daha yüksek çıktı. 

Model seçimi tamamlandıktan sonra `{tune}` paketindeki `last_fit()`  fonksiyonu ile  

1. eğitim-test ayrımını yapabilir

2. eğitim verisiyle modeli tahmin edebilir

3. kestirimleri oluşturup değerlendirme ölçütlerini hesaplayabilir

4. ve bunları bir nesne içinde saklayabiliriz. 
...
`tune::collect_metrics()` fonksiyonu model değerlendirme ölçütlerini hesaplar: 
```{r}
lm_last_fit %>% 
  collect_metrics()
```

`tune::collect_predictions()` gerçek ve kestirim değerlerini içeren bir tibble oluşturur: 
```{r}
lm_last_fit %>% 
  collect_predictions()
```
...
Tidymodels ile k-katlı çapraz geçerleme yapılabilir. 
...
k=5 için CV katlarının oluşturulması: 
```{r}
set.seed(12)
...
Bir akış (workflow) oluşturup tüm katlar için tahminleri yapabiliriz: 
```{r}
auto_wf <- workflow() %>% 
  add_model(lm_model) %>% 
...
```
Yukarıdaki tabloda yer alan metrikler 5 kat için ortalamalardır. 
