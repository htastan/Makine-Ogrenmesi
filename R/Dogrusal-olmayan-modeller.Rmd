...
Yapay sinir ağları için çeşitli R kütüphaneler bulunmaktadır. Bunlardan biri `{neuralnet}` paketidir (bkz. [neuralnet CRAN page](https://cran.r-project.org/web/packages/neuralnet/index.html)). 
...
Şimdi simülasyon ile oluşturduğumuz veri setinden hareketle train-test ayırımı yapalım. Bundan önce, değişkenleri 0-1 aralığına olacak şekilde standardize edelim: 
```{r}
scale1 <- function(x){
  (x - min(x)) / (max(x) - min(x))
}
dfs <- df %>% mutate_all(scale1)
```
...
nnet4 <- neuralnet(y ~ x1 + x2 + x3, data=dfs_train, hidden=c(3,3))
plot(nnet4, rep = "best")
```
...
```{r}
options(scipen = 999) # bilimsel notasyonu kapat
...
```{r}
data_pred <- tibble(pred_nnet4, pred_linreg, pred_linreg2, truth = dfs_test$y)
...
p3 <- ggplot(data_pred, aes(truth, pred_linreg2)) + 
  geom_point() + 
  geom_smooth() +
  ggtitle("Etkileşimli lineer reg")
# plot 
library(patchwork)
p1 | p2 | p3
```
...
# scale1 fonksiyonu yukarıda tanımlanmıştı
```
...
set.seed(1010)
nn1 <- neuralnet(medv ~ crim + zn + indus + chas + nox + rm + age + dis + rad + tax + 
                   ptratio + black + lstat,
                 data = boston_train,
                 hidden = c(5,5))
```
...
```{r}
# tüm değişkenlerle lineer regresyon 
