...
`{dplyr}` kütüphanesi verilerin işlenmesi ve manipülasyonu için çeşitli fonkisyonlar içerir. Bunların en önemlileri aşağıda ele alınmıştır. 
...
```{r, echo=TRUE}
ggplot(data = not_verileri) +
  geom_boxplot(mapping = aes(x = ders, y=not))  
```
